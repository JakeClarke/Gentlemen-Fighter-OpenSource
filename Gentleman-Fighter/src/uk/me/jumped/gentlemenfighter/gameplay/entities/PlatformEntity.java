...
public class PlatformEntity extends Entity {
	...
	public PlatformEntity(float posX, float posY, float width, float height, Frame[] frames,
			EntityManager parentManager) {
		super(genPlatformBodyDef(posX, posY, width, height), genFixtureDef(width, height), parentManager);
		...
		this.platformSprite.Batch = parentManager.getGameplayScreen().getMidSpriteBatch();
	...
	private static BodyDef genPlatformBodyDef(float posX, float posY, float width, float height) {
